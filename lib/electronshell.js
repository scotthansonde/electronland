...
exports.openLinksInExternalWindow = openLinksInExternalWindow; //8/10/17 by DW
exports.chooseFolderDialog = shellChooseFolderDialog; //8/29/17 by DW
exports.openItem = shellOpenItem; //9/7/17 by DW
exports.openLinkInExternalWindow = openLinkInExternalWindow; //10/24/17 by DW
exports.buildTabs = buildTabs; //2/16/20 by DW
...
function openLinkInExternalWindow (url) { //10/24/17 by DW
...
function shellOpenItem (filepath) { //9/7/17 by DW
	require ("electron").ipcRenderer.send ("asynch-message", "openItem", filepath);  
	}
...
function shellOpenFileDialog (callback) {
	...
	var theFiles = electron.remote.dialog.showOpenDialogSync (options);
	if (theFiles !== undefined) {
		console.log ("shellOpenFileDialogSync: theFiles == " + theFiles);
		if (callback !== undefined) {
			callback (theFiles);
			}
		}
	}
function shellChooseFolderDialog (defaultFolder, callback) { //8/29/17 by DW
	var options = {
		...
		for (var i = 0; i < prefs.myTabs.length; i++) {
			...
			//set icon
				if (tab.flInstantOutline) {
					...
					icon = "far fa-file-alt"; //1/30/20 by DW
					}
				icon = "<i class=\"" + icon + updateWaitingClass + "\"></i>";
			//set title
